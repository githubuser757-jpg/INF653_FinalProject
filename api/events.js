...
eventsRouter.post('/bookings', verifyToken, async(req, res)=>{
    try{
        ...
        if(!eventId || !quantity){
            return res.status(401).json({error: 'EventId, password, and role are required.'});
        }
...
eventsRouter.put('/events/:id', verifyToken, async(req, res)=>{
    try{
        ...
        //Valid id is a MongoDB ObjectId
        if(!mongoose.Types.ObjectId.isValid(id)){
        ...
        if(bookedSeats){
            if(bookedSeats > event.seatCapacity){
                return res.status(400).json({error: 'bookedSeats cannot be greater than seat capacity.'});
              }
            event.bookedSeats = bookedSeats || event.bookedSeats;
          }
